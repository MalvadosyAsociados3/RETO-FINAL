"""
MINI CHALLENGE 6 — Bug 0: Gazebo + reactive navigation (all-in-one).
...
Launches:
  ...
  4. bug0_launch.py (localisation + point_generator + bug0 + RViz)

Usage:
  ros2 launch puzzlebot_sim mc6_bug0_launch.py
  ros2 launch puzzlebot_sim mc6_bug0_launch.py world:=obstacle_avoidance_2.world
  ros2 launch puzzlebot_sim mc6_bug0_launch.py use_rviz:=false
"""
import os
...
def generate_launch_description():
    ...
    # ── Bug 0 stack (localisation + point_generator + bug0 + rviz) ─────
    bug0_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_sim, 'launch', 'bug0_launch.py')
        ),
        launch_arguments={
            'use_sim_time': use_sim_time,
